...
public class XOmdaGradlePlugin implements Plugin<Project> {
    ...
    public void apply(Project project) {
        ...
        org.gradle.api.artifacts.Configuration conf = project
            .getConfigurations()
        ...
        project.getBuildscript().getConfigurations()
            ...
            .filter((Dependency dep) -> dep.getName().startsWith(XOMDA_GROUP))
            .findFirst()
            .ifPresent(d -> {
        ...
        XOmdaGradlePluginExtension extension = project.getExtensions().create(Constants.XOMDA_CONFIGURATION, XOmdaGradlePluginExtension.class);
        extension.getClasspath().convention(Set.of(Configuration.DEFAULT_CLASSPATH));
        extension.getModels().convention(omdaSourceSet
            .getResources()
            .getFiles()
            .stream()
