		...
		JavaClassWriter ctx = new JavaClassWriter(fullyQualifiedName)
			...
			ctx.println("public interface " + interfaceName + " {").tab(tabbed -> tabbed.println()
					...
					.forEach(entity::getAttributeList, (final Attribute attribute) -> {
						...
						final String identifier = TemplateUtils
								.getJavaIdentifier(StringUtils.toCamelCase(attribute.getName()));
						tabbed
								// getter
								.addDocs(doc -> {
					...
					.forEach(findReverseEntities(entity), (final Entity e) -> {
						...
						final CharSequence fullyQualifiedType = WritableContext.format("{0}<{1}>",
								ctx.addImport(List.class), ctx.addImport(TemplateUtils.getJavaType(e)));
