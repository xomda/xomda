...
public class JavaUtils {
    ...
    private static final Collection<String> JAVA_RESERVED_KEYWORDS = Set.of(
        ...
        "break", "double", "implements", "public", "throw",
    ...
    private static final Collection<String> JAVA_RESERVED_IDENTIFIER = Set.of(
        "exports", "module", "non-sealed", "open", "opens", "permits",
        "provides", "record", "requires", "sealed", "to", "transitive",
        "uses", "var", "when", "with", "yield");

    private static final Collection<String> JAVA_RESERVED_LITERALS = Set.of(
        "true", "false", "null");

    private static final Collection<String> JAVA_RESERVED_CONSTANTS = Set.of(
        "STR", "FMT", "RAW");

    /**
     * Checks if the given "word" is reserved in the Java Language (ie. <code>if</code>,
     * <code>then</code>, <code>else</code>, ...).
     * If so, it should obviously not be used as identifier in the generated code, for example.
     */
    public static boolean isReserved(final String word) {
        return JAVA_RESERVED_LITERALS.contains(word)
            || JAVA_RESERVED_IDENTIFIER.contains(word)
            || JAVA_RESERVED_KEYWORDS.contains(word)
            || JAVA_RESERVED_CONSTANTS.contains(word);
    }

    public static String toIdentifier(final String in) {
        return !isReserved(in)
            ? in
            : toIdentifier(in.charAt(0) + in.substring(1).replaceAll("[aeiuop]+", ""));
    }

    public static <A, B> boolean isSamePackage(final Class<A> a, final Class<B> b) {
        return hasPackage(a) == hasPackage(b)
            && getPackageName(a).equals(getPackageName(b));
    }
    ...
    public static String getPackageName(final String fullyQualifiedClassName) {
        ...
        return fullyQualifiedClassName.substring(
            0, fullyQualifiedClassName.lastIndexOf('.'));
    }
    ...
    public static void validateClassName(final String className, final boolean withPackage) {
        final boolean validPackage = withPackage && hasPackage(className) && isValidPackageName(getPackageName(className));
    ...
    public static boolean isGlobal(final String className) {
        return hasPackage(className)
            && ("java.lang".equals(className) || className.startsWith("java.lang.") && className.lastIndexOf('.') == 9);
    }
